...
- For REGRESSION (like GFP yield): Think about biochemical processes - which factors are limiting? Which interact multiplicatively?
- Reason about relationships: "If class A has high feature X and class B has low feature X, then feature X discriminates" (classification)
- For GFP yield: "If NTPs is low, yield should be low regardless of other factors - NTPs is a critical limiting factor"
...
- Consider domain-specific knowledge: 
  * For vehicles: think about what makes buses vs vans vs sedans distinctive
  * For GFP yield: think about biochemical processes - temperature optimum, saturation kinetics, synergistic interactions
- Create meaningful intermediate variables: Model domain concepts (e.g., "boxiness", "elongation_factor", "ribosome_stability", "transcription_rate")
...
2. LEARN NEW COEFFICIENT VALUES (CRITICAL - MAKE SIGNIFICANT CHANGES): 
   Your current formula has coefficients (like 0.783, 0.301, 0.194, etc.) that are starting values. 
   You MUST change these SIGNIFICANTLY based on the learning examples and performance metrics.
   
   SYSTEMATIC COEFFICIENT OPTIMIZATION:
   - Don't make tiny changes (0.8 → 0.9) - make meaningful changes (0.8 → 1.5 or 2.0)
   - Try these coefficient ranges systematically:
     * Base scaling: 0.5, 1.0, 1.5, 2.0, 2.5, 3.0
     * Saturation K values: 0.05, 0.1, 0.2, 0.3, 0.5, 0.8
     * Interaction scaling: 0.5, 1.0, 1.5, 2.0
   - If R² < 0.5: Increase important feature coefficients by 2-3x (e.g., 0.3 → 0.9 or 1.2)
   - If MAE > 0.15: Adjust coefficients to better match TARGET values from learning examples
   - For each learning example: Calculate what your formula predicts, compare to TARGET, adjust coefficients
   - Don't just add interaction terms - actually change the base coefficients significantly
   
   REASON ABOUT COEFFICIENTS: 
   - "If feature X is very important for class A, its coefficient should be larger (try 1.5-2.5x current)"
   - "If learning examples show high Feature Y → high TARGET, increase Feature Y coefficient"
   - "If my formula predicts 0.5 but TARGET is 0.75, I need to increase coefficients"
...
4. USE LEARNING EXAMPLES TO IMPROVE INDEPENDENT PERFORMANCE (MOST IMPORTANT):
   The learning examples above show EXACT input-output pairs: features → TARGET value.
   These are your PRIMARY learning signal. Use them systematically:
   
   FOR EACH EXAMPLE:
   - Calculate what your current mechanism would predict
   - Compare to the TARGET value - identify the gap
   - Adjust coefficients/interactions to close the gap
   - Verify: Does your new formula predict close to TARGET for this example?
   
   PATTERN IDENTIFICATION:
   - Which features correlate with high TARGET values? → Increase their coefficients
   - Which features correlate with low TARGET values? → Ensure they have appropriate influence
   - Are there feature combinations that lead to specific TARGET values? → Add interaction terms
   
   SYSTEMATIC ADJUSTMENT:
   - If Example shows [Feature A=0.8, Feature B=0.2] → TARGET=0.75:
     * Your formula should output ~0.75 for these feature values
     * If it outputs 0.5, increase coefficients for features that should push it higher
     * If it outputs 0.9, decrease coefficients or add inhibition terms
   - Adjust coefficients iteratively until examples are matched
   
   REASON ABOUT ERRORS: 
   - "The ML model predicts 0.6 but TARGET is 0.75 - I need to increase coefficients"
   - "Learning examples show high Feature X → high TARGET, so Feature X coefficient should be larger"
   - "My formula gives 0.5 for this example but TARGET is 0.8 - I need significant coefficient changes"
...
5. CRITICAL: ADD ADVANCED NONLINEAR TRANSFORMATIONS AND LEARN NONLINEAR INTERACTIONS:
   ...
   - Use saturation effects: feature / (K + feature) to capture diminishing returns (Michaelis-Menten kinetics for biochemical systems)
   ...
   - FOR GFP YIELD: Use multiplicative interactions for synergistic effects (Mg × K+ for ribosome stability), add inhibition terms for resource competition (high DNA)

...
try:
    from maicl.maicl_config import (
        MAX_TOP_FEATURES,
        MAX_WORST_INDICES,
...
class TextGrad:
    ...
    def _build_enhanced_error_feedback(self, X_train, y_train, ml_residuals, current_loss, dataset_name, 
        ...
        CRITICAL: This function includes residual samples as LEARNING EXAMPLES with TARGET values.
        The LLM will learn from these examples to predict the correct target values.
        All residual samples come from TRAINING DATA ONLY (X_train, y_train).
...
""")
        ...
        # CRITICAL: These residual samples come from TRAINING DATA ONLY (X_train, y_train)
        # They can be used as learning examples, not just error feedback
        if ml_residuals is not None and len(ml_residuals) > 0:
            # Verify residuals match training data length (safety check)
            if len(ml_residuals) != len(X_train) or len(ml_residuals) != len(y_train):
                logger.warning(f"[TextGrad] Residual length ({len(ml_residuals)}) doesn't match training data (X: {len(X_train)}, y: {len(y_train)}). Skipping residual examples.")
            else:
                # Get worst predictions - use ALL residuals (they're already filtered to top_k if from train_on_residuals)
                # The sample size is determined ONLY by top_k (which is len(ml_residuals) when filtered)
                # No hardcoded limit - use all residuals provided
                num_examples = len(ml_residuals)  # Use all top_k examples (no MAX_WORST_INDICES limit)
                worst_idx = np.argsort(np.abs(ml_residuals))[::-1][:num_examples]
                ...
                # Add note that these are learning examples from training data
                feedback += "\n" + "="*80 + "\n"
                feedback += "📚 LEARNING EXAMPLES (from training data) - YOUR PRIMARY LEARNING SIGNAL:\n"
                feedback += "="*80 + "\n"
                feedback += "⚠️ CRITICAL: These examples show EXACT input-output pairs you must learn.\n"
                feedback += "Each example shows: Features → TARGET Value (what you MUST predict)\n"
                feedback += "YOUR TASK: Adjust your mechanism's formula so it predicts the TARGET value for each example.\n\n"
                feedback += "HOW TO USE THESE EXAMPLES:\n"
                feedback += "1. For each example, calculate what your current mechanism would predict\n"
                feedback += "2. Compare to the TARGET value - identify the gap\n"
                feedback += "3. Adjust coefficients/interactions to close the gap\n"
                feedback += "4. Verify: Does your new formula predict close to TARGET for this example?\n"
                feedback += "5. Repeat for all examples - your formula should match ALL target values\n\n"
                feedback += "PATTERN IDENTIFICATION:\n"
                feedback += "- Look for patterns: Which features correlate with high TARGET values?\n"
                feedback += "- Which features correlate with low TARGET values?\n"
                feedback += "- Are there feature combinations that lead to specific TARGET values?\n"
                feedback += "- Use these patterns to guide your coefficient and interaction choices.\n\n"
            ...
            # For DeepChem: filter out ecfp_bit_ features from feature_names
            # For non-DeepChem: use all features
            if is_deepchem:
                # Filter out ecfp_bit_ features for DeepChem datasets
                feature_names = [f for f in (self.feature_cols if self.feature_cols else [f"x{j}" for j in range(X_train.shape[1])]) 
                               if not str(f).startswith("ecfp_bit_")]
            else:
                # For non-DeepChem: use ALL features
                feature_names = self.feature_cols if self.feature_cols else [f"x{j}" for j in range(X_train.shape[1])]
            
            for example_num, idx in enumerate(worst_idx, 1):
                ...
                if use_smiles and idx < len(X_train_original):
                    ...
                    if isinstance(original_feat, dict) and 'SMILES' in original_feat:
                        # Format SMILES with ALL descriptors (not just 5)
                        key_feats_str = f"SMILES={original_feat['SMILES']}"
                        # Add ALL non-ECFP descriptors if available
                        try:
                            extra_items = []
                            for k, v in original_feat.items():
                                if k == "SMILES" or str(k).startswith("ecfp_bit_"):
                                    continue
                                # Show all descriptors, not limited to MAX_FEATURES_IN_EXAMPLE
                                extra_items.append(f"{k}={v:.2f}" if isinstance(v, (int, float, np.number)) else f"{k}={str(v)[:20]}")
                            if extra_items:
                                key_feats_str += ", " + ", ".join(extra_items)
                        except Exception:
                            pass
                    else:
                        # Fallback: use filtered feature_names (already excludes ecfp_bit_ for DeepChem)
                        # Map feature_names to X_train columns (need to find indices)
                        if is_deepchem:
                            # For DeepChem fallback, we need to map filtered feature names back to X_train indices
                            all_feature_names = self.feature_cols if self.feature_cols else [f"x{j}" for j in range(X_train.shape[1])]
                            feat_vals = {}
                            for feat_name in feature_names:
                                if feat_name in all_feature_names:
                                    feat_idx = all_feature_names.index(feat_name)
                                    if feat_idx < X_train.shape[1]:
                                        feat_vals[feat_name] = float(X_train[idx, feat_idx])
                        else:
                            # For non-DeepChem: use all features directly
                            feat_vals = {feature_names[j]: float(X_train[idx, j]) 
                                        for j in range(min(len(feature_names), X_train.shape[1]))}
                        key_feats_str = ", ".join([f"{k}={v:.2f}" for k, v in list(feat_vals.items())])
                else:
                    # For non-DeepChem: show ALL features (feature_names already filtered for DeepChem)
                    if is_deepchem:
                        # For DeepChem without SMILES: map filtered feature names to X_train indices
                        all_feature_names = self.feature_cols if self.feature_cols else [f"x{j}" for j in range(X_train.shape[1])]
                        feat_vals = {}
                        for feat_name in feature_names:
                            if feat_name in all_feature_names:
                        ...
                        feat_vals = {feature_names[j]: float(X_train[idx, j]) 
                                    for j in range(min(len(feature_names), X_train.shape[1]))}
                    # Format features in a structured way - show ALL features (excluding ecfp_bit_ for DeepChem)
                    key_feats_str = ", ".join([f"{k}={v:.2f}" for k, v in list(feat_vals.items())])
                
                if self.task_type == "regression":
                    y_pred_val = ml_residuals[idx] + y_train[idx]
                    target_val = y_train[idx]
                    error_val = ml_residuals[idx]
                    # Format as structured learning example with clear columns
                    feedback += f"Example #{example_num}:\n"
                    feedback += f"  Features: {key_feats_str}\n"
                    feedback += f"  ML Prediction: {y_pred_val:.4f} (ML model's guess - often wrong)\n"
                    feedback += f"  TARGET Value: {target_val:.4f} ← YOUR MECHANISM MUST PREDICT THIS EXACT VALUE\n"
                    feedback += f"  ML Error: {error_val:+.4f} (ML was {'too high' if error_val < 0 else 'too low'})\n"
                    feedback += f"  YOUR TASK: Design your formula so that when features = [{key_feats_str}], output = {target_val:.4f}\n"
                    feedback += f"  VERIFY: Calculate your formula with these feature values - does it give ~{target_val:.4f}?\n\n"
                else:
                    ...
                    # Format classification examples in structured format (same as regression)
                    feedback += f"Example #{example_num}:\n"
                    feedback += f"  Features: {key_feats_str}\n"
                    
                    if class_names and len(class_names) > 0:
                        target_name = class_names[true_idx] if 0 <= true_idx < len(class_names) else str(true_idx)
                        if pred_idx >= 0 and pred_idx < len(class_names):
                            pred_name = class_names[pred_idx]
                            feedback += f"  ML Prediction: {pred_name}\n"
                        else:
                            feedback += f"  ML Prediction: (unknown - misclassified)\n"
                        feedback += f"  TARGET Class: {target_name} ← YOUR MECHANISM MUST PREDICT THIS\n"
                        feedback += f"  Status: {match_str}\n"
                        feedback += f"  Learning Goal: When features are [{key_feats_str}], your mechanism should output class '{target_name}'\n\n"
                    else:
                        if pred_idx >= 0:
                            feedback += f"  ML Prediction: {pred_idx}\n"
                        else:
                            feedback += f"  ML Prediction: (unknown - misclassified)\n"
                        feedback += f"  TARGET Class: {true_idx} ← YOUR MECHANISM MUST PREDICT THIS\n"
                        feedback += f"  Status: {match_str}\n"
                        feedback += f"  Learning Goal: When features are [{key_feats_str}], your mechanism should output class {true_idx}\n\n"
                
                # Add summary after all examples to reinforce learning
                if len(worst_idx) > 0:
                    feedback += "="*80 + "\n"
                    feedback += "SUMMARY - WHAT TO LEARN FROM THESE EXAMPLES:\n"
                    feedback += "="*80 + "\n"
                    if self.task_type == "regression":
                        feedback += "SYSTEMATIC LEARNING PROCESS:\n"
                        feedback += "1. For EACH example above, calculate what your current mechanism predicts\n"
                        feedback += "2. Compare prediction to TARGET value - calculate the error\n"
                        feedback += "3. Identify patterns: Which features correlate with high/low TARGET values?\n"
                        feedback += "4. Adjust coefficients systematically:\n"
                        feedback += "   - If high Feature X → high TARGET: increase Feature X coefficient\n"
                        feedback += "   - If low Feature Y → low TARGET: ensure Feature Y has strong influence\n"
                        feedback += "   - If Feature A + Feature B → specific TARGET: add interaction term\n"
                        feedback += "5. Try coefficient values: 0.5, 1.0, 1.5, 2.0, 2.5, 3.0 (not just 0.8-1.2)\n"
                        feedback += "6. VERIFY: For each example, does your new formula predict close to TARGET?\n"
                        feedback += "7. If not, adjust coefficients further until examples are matched\n"
                        feedback += "8. Your goal: R² > 0.7, MAE < 0.1 when mechanism is used alone\n"
                    else:
                        feedback += "1. Each example shows a specific feature combination and its TARGET class.\n"
                        feedback += "2. Your mechanism must learn to predict the TARGET class when it sees similar feature combinations.\n"
                        feedback += "3. Study the patterns: which features lead to which TARGET classes?\n"
                        feedback += "4. Adjust your mechanism's decision boundaries to correctly classify these examples.\n"
                        feedback += "5. Test your mechanism mentally: for each example, would your formula output the TARGET class?\n"
                    feedback += "\n"
    ...
    def _build_iteration_history_context(
        ...
        # Log that iteration history is being included (for debugging)
        import logging
        logger = logging.getLogger(__name__)
        accepted_in_history = sum(1 for h in filtered if h.get('accepted', False))
        rejected_in_history = len(filtered) - accepted_in_history
        logger.debug(f"  [TextGrad History] Including {len(filtered)} iteration(s) of history in prompt (accepted: {accepted_in_history}, rejected: {rejected_in_history})")
        ...
        # Add warning if there are rejected iterations
        if rejected_streak > 0:
            escalation_warning = f"\n⚠️ CRITICAL: {rejected_streak} consecutive rejection(s) detected. You MUST make SIGNIFICANT changes:\n"
            escalation_warning += "- Change at least 2-3 coefficients by 50-100% or more (not tiny tweaks)\n"
            escalation_warning += "- Add or modify interaction terms or nonlinear transformations\n"
            escalation_warning += "- Make substantial structural changes to the formula, not just coefficient adjustments\n"
            escalation_warning += "- Focus on matching TARGET values from learning examples - adjust coefficients systematically\n"
            context_parts.append(escalation_warning)
    ...
    def optimize_batch_single_call(self, mechanisms: List[str], error_feedbacks: List[str],
        ...
        if (X_train is not None and y_train is not None and 
            ...
            try:
                ...
                # Verify residual samples are included in error feedback
                if ml_residuals is not None and len(ml_residuals) > 0:
                    # Use ALL residuals - sample size determined ONLY by top_k (len(ml_residuals))
                    # No hardcoded MAX_WORST_INDICES limit - use all residuals provided
                    num_examples = len(ml_residuals)  # Use all top_k examples
                    logger.info(f"  [TextGrad] ✓ Residual samples ({len(ml_residuals)}) are included in error feedback as learning examples")
                    logger.info(f"  [TextGrad] ✓ LLM will see {num_examples} worst residual examples with TARGET values to learn from (top_k={len(ml_residuals)})")
                else:
                    logger.warning(f"  [TextGrad] ⚠️ No residual samples provided - LLM won't see specific learning examples")
            except Exception as e:
        ...
        for mech_local_idx, (mechanism, error_feedback) in enumerate(zip(mechanisms, enhanced_feedbacks)):
            ...
            # Add Chain-of-Thought reasoning prompt if this is not the first iteration
            cot_prompt = ""
            if iteration_history and len(iteration_history) > 0:
                cot_prompt = """

🧠 CHAIN-OF-THOUGHT REASONING (CRITICAL FOR THIS ITERATION):

Since previous iterations may have been rejected or showed limited improvement, you MUST think more deeply:

1. REASON ABOUT THE PROBLEM STRUCTURE:
   - Is this a single-step prediction, or should it be multi-step with intermediate variables?
   - What intermediate concepts would help? (e.g., "ribosome_stability", "transcription_rate", "temperature_effectiveness")
   - How do these intermediates combine to predict the final TARGET?

2. ANALYZE EACH LEARNING EXAMPLE SYSTEMATICALLY:
   - Write out: "Example #1: Features = [...], TARGET = X.XX"
   - Calculate: "My current formula gives Y.YY, error is ±Z.ZZ"
   - Reason: "To match TARGET, I need to [increase/decrease] [which coefficient/term] by [how much]"
   - Repeat for all examples

3. DESIGN INTERMEDIATE VARIABLES (if needed for complex interactions):
   - Think: "What intermediate concept captures the pattern I see?"
   - Example: intermediate1 = feature1 * feature2 / (K + feature1 * feature2)  # synergistic saturation
   - Example: intermediate2 = exp(-((feature3 - optimum)^2) / (2*width^2))  # Gaussian optimum
   - Combine: ŷ = base * intermediate1 * intermediate2 * ...

4. SYSTEMATIC COEFFICIENT OPTIMIZATION:
   - For each coefficient, reason: "Current value is X, examples suggest Y, so I'll try Z"
   - Don't make tiny changes - make meaningful changes (0.8 → 1.5, not 0.8 → 0.9)
   - Verify: "With new coefficients, Example #1 gives ~TARGET ✓"

5. THINK ABOUT GENERALIZATION:
   - Does your formula make sense for feature values NOT in the examples?
   - Are there edge cases that might break your formula?
   - Add safeguards: saturation, clipping, threshold effects

USE THIS REASONING PROCESS IN YOUR OUTPUT - show your thinking step by step.
"""
                # Append CoT prompt to iteration history context
                iteration_history_context = iteration_history_context + cot_prompt
                # Append CoT prompt to iteration history context
                iteration_history_context = iteration_history_context + cot_prompt

...
class MLModelMechanism:
    ...
    def __init__(self, model_name: str = "LinearRegression", task_type: str = "classification"):
        ...
        self._y_scaler = None  # Store y_scaler to know if targets are scaled
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, feature_cols: List[str], y_scaler: Any = None):
        ...
        self._y_scaler = y_scaler  # Store y_scaler to detect if targets are scaled
        ...
        if self.model_name == "LinearRegression":
            ...
            # EBM (Explainable Boosting Machine) - supports both classification and regression
            try:
                ...
                os.environ.setdefault('EBM_LOG_LEVEL', 'ERROR')
                
                if self.task_type == "regression":
                    from interpret.glassbox import ExplainableBoostingRegressor  # type: ignore
                    self.model = ExplainableBoostingRegressor(random_state=RANDOM_STATE, n_jobs=1)
                else:
                    from interpret.glassbox import ExplainableBoostingClassifier  # type: ignore
                    self.model = ExplainableBoostingClassifier(random_state=RANDOM_STATE, n_jobs=1)
                ...
                logger.info("✓ EBM (Explainable Boosting Machine) initialized successfully")
            except ImportError as e:
                raise RuntimeError(
                    f"EBM not installed or failed to import. Install with: pip install interpret. "
                    f"Original error: {e}"
                ) from e
            except Exception as e:
                raise RuntimeError(f"EBM initialization failed: {e}") from e
        else:
        ...
        if self.task_type == "classification":
            ...
            # TabICL and EBM typically have predict_proba, so skip calibration for them
            ...
            elif self.model_name not in ("TabICL", "EBM") and not hasattr(self.model, "predict_proba"):
                try:
    ...
    def predict(self, x_dict: Dict[str, float], return_class_index: bool = False) -> float:
        ...
        if getattr(self, "task_type", "classification") == "regression":
            if self.model_name == "TabICL" and hasattr(self, '_tabicl_bin_centers') and self._tabicl_bin_centers is not None:
                ...
                # For EBM, clip predictions if targets are scaled (EBM can extrapolate)
                if self.model_name == "EBM" and self._y_scaler is not None:
                    prediction = float(np.clip(prediction, SCALE_MIN, SCALE_MAX))
                return float(prediction)

        is_multi_class = (hasattr(self.model, 'classes_') and len(self.model.classes_) > 2)
        
        if self.model_name in ("LogisticRegression", "XGBoost", "EBM"):
            ...
            if self.task_type == "classification":
                ...
                # Regression: This should not be reached for EBM (handled above), but keep for safety
                # EBM regression is already handled in the regression block above
                prediction = self.model.predict(x_array)[0]
                # EBM can extrapolate beyond training range, so clip if targets are scaled
                if self.model_name == "EBM" and self._y_scaler is not None:
                    # Clip to scaled range (typically [0.0, 1.0])
                    prediction = float(np.clip(prediction, SCALE_MIN, SCALE_MAX))
                return float(prediction)

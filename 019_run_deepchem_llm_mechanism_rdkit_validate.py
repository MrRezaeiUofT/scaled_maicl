...
def get_encoder_prompt_v2(dataset_key: str) -> str:
    """
    Generate an improved encoder prompt with chain-of-thought structure.
    """
...
Your output will guide a formula generator to create an interpretable, executable mechanism.
...
### Step 2: Direction of Effects
For each descriptor group, determine: does increasing this descriptor INCREASE or DECREASE the target?
Be explicit: "num_rings: INCREASES hydrophobicity → DECREASES solubility" (for ESOL)

### Step 3: Nonlinear Effects
Consider saturation effects: very high values of a descriptor may have diminishing returns.
Consider threshold effects: some descriptors may only matter above/below certain values.
...
1. Primary drivers of the target property
2. Expected direction of each descriptor's effect
3. Key nonlinearities or interactions to include
"""
...
### 4. RDKitValidation (REQUIRED)
This MUST be present and formatted exactly as shown:
```
RDKitValidation:
{"expected_effects":{"num_rings":"positive","is_aromatic":"positive","num_halogen":"positive","molecular_weight":"positive","num_atoms":"neutral","num_oxygen":"negative","num_nitrogen":"negative","num_hydroxyl_groups":"negative","num_hbd":"negative"}}
```
...
### 5. Formula (REQUIRED - MUST BE LAST)
```
...
def main() -> None:
    ...
    ap.add_argument("--k_shot", type=int, default=10, help="Few-shot examples per prediction inside MA-ICL train/eval.")

...
# -*- coding: utf-8 -*-
"""
Run Classification Parameter Sweep
-----------------------------------
Runs 017_maicl_classification_residual_topk.py with different parameter combinations.

Combinations:
- top_k: 30, 50, 75, 100
- ml_mech: logreg, xgboost
- scaling: scaling (default), no_scaling
- acceptance_set: test, validation
- num_mechanisms_unknown: 1, 2
- classification_loss: f1, acc
- topk_strategy: residual, residual_balanced

Total: 4 * 2 * 2 * 2 * 2 * 2 * 2 = 256 combinations
"""
...
PARAM_COMBINATIONS = {
    ...
    'acceptance_set': ['test', 'validation'],
    'num_mechanisms_unknown': [1, 2],
    'classification_loss': ['f1'],
    'topk_strategy': ['residual'],
}

# Generate log file name with dataset extension
LOG_FILE = f'run_classification_sweep_{BASE_ARGS["dataset"]}.log'
...
def build_command(args_dict):
    ...
    cmd = ['python', '017_maicl_classification_residual_topk.py']
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("Starting Classification Parameter Sweep")
    logger.info("=" * 80)
    ...
    for idx, combo_dict in enumerate(all_combinations, 1):
        ...
        results_file = 'run_classification_sweep_results.json'
    ...
    try:
        ...
        csv_file = 'run_classification_sweep_summary.csv'

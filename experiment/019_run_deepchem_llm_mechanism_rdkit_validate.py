...
from maicl import maicl_config
from maicl.maicl_lib_v2 import MinMaxScaler010, TrainableMAICL
...
def compute_sensitivity_analysis(
    ...
    for desc_name in SUPPORTED_RDKIT_DESCRIPTOR_NAMES:
        ...
        perturbation = desc_values * perturbation_pct
        perturbation = np.maximum(perturbation, 0.1)  # Minimum perturbation
        perturbed_desc[desc_name] = desc_values + perturbation
        
        # Recompute descriptors for perturbed values (simplified: just modify the descriptor dict)
        # In practice, we'd need to modify SMILES and recompute, but this is a proxy
        try:
            perturbed_X = np.zeros((len(X_original), len(feature_cols)))
            # Note: This is a simplified sensitivity analysis
            # A full version would require modifying SMILES and recomputing descriptors
            perturbed_preds = baseline_preds  # Placeholder - would need full recomputation
            sensitivity = float(np.mean(np.abs(perturbed_preds - baseline_preds)))
        except Exception:
            sensitivity = 0.0
        
        sensitivities[desc_name] = sensitivity
...
def _validate_esol_signs(formula: str, dataset_key: str) -> Tuple[bool, Optional[str]]:
    ...
    # This is a heuristic - look for patterns like "+ 0.5*num_halogen" (wrong) vs "- 0.5*num_halogen" (correct)
    violations = []
    ...
    for desc_name, expected_sign in expected_signs.items():
        ...
        # Look for the sign before the descriptor (accounting for parentheses and operations)
        # Pattern: look for [+-] followed by optional coefficient and the descriptor
        ...
        if not matches:
            ...
            # This is harder to detect, so we'll be lenient
            continue
    ...
    if violations:
        return False, f"Sign violations detected: {', '.join(violations)}"
...
def main() -> None:
    ...
    # First load without max_samples to get all data, then combine and apply max_samples, then split
    (
        (X_train_full, y_train_full, X_original_train_full),
        (X_val_full, y_val_full, X_original_val_full),
        (X_test_full, y_test_full, X_original_test_full),
        feature_cols,
        feature_encoders,
        ds_label,
    ) = runner.load_deepchem_regression_dataset(
        args.dataset,
        None,  # Don't apply max_samples yet - load all data
    ...
    # Combine all splits
    X_all = np.vstack([X_train_full, X_val_full, X_test_full])
    y_all = np.concatenate([np.asarray(y_train_full), np.asarray(y_val_full), np.asarray(y_test_full)])
    X_original_all = X_original_train_full + X_original_val_full + X_original_test_full
    
    maicl_config.logger.info(f"Combined all splits: {len(X_all)} total samples")
    ...
    # ---------------------------------------------------------------------
    # DeepChem safety shim + LLM-only evaluation for acceptance (EXPERIMENT-SPECIFIC):
    ...
    # 
    # IMPORTANT: For this experiment, we use LLM-only evaluation for acceptance decisions
    # instead of ensemble evaluation, since we're in LLM-only mode (no ML mechanism).
    # ---------------------------------------------------------------------
    _orig_evaluate = maicl.evaluate
    _orig_evaluate_llm_only = maicl.evaluate_llm_only

    def _evaluate_with_smiles_autofill_and_llm_only(X, y_true, X_pool, y_pool, *args, **kwargs):
        ...
        # Check if we're evaluating on the acceptance set (for acceptance decisions)
        # This experiment uses LLM-only evaluation for acceptance decisions
        is_acceptance_set = False
        acceptance_set_X = getattr(maicl, "_acceptance_set_X", None)
        acceptance_set_X_original = getattr(maicl, "_acceptance_set_X_original", None)
        
        if acceptance_set_X is not None:
            # Check by array comparison (shape and values)
            if X.shape == acceptance_set_X.shape and len(X) == len(acceptance_set_X):
                try:
                    is_acceptance_set = np.allclose(X, acceptance_set_X, rtol=1e-10, atol=1e-10)
                except Exception:
                    pass
        
        # Also check by X_original if available (more robust for DeepChem)
        if not is_acceptance_set and is_deepchem and acceptance_set_X_original is not None:
            X_orig = kwargs.get("X_original", None)
            if X_orig is not None and len(X_orig) == len(acceptance_set_X_original):
                # Check if it's the same set by comparing first few SMILES
                if len(X_orig) > 0 and len(acceptance_set_X_original) > 0:
                    try:
                        if isinstance(X_orig[0], dict) and isinstance(acceptance_set_X_original[0], dict):
                            is_acceptance_set = (X_orig[0].get("SMILES", "") == acceptance_set_X_original[0].get("SMILES", ""))
                        else:
                            is_acceptance_set = (str(X_orig[0]) == str(acceptance_set_X_original[0]))
                    except Exception:
                        pass
        ...
        # For this experiment: use LLM-only evaluation for acceptance set decisions
        # BUT only if LLM mechanisms exist (skip for initial checkpoint when no LLM mechanisms yet)
        if is_acceptance_set:
            # Check if there are any LLM mechanisms
            has_llm_mechanisms = any(
                getattr(maicl, "mechanism_types", []) and 
                mech_type == "llm" 
                for mech_type in getattr(maicl, "mechanism_types", [])
            )
            
            if has_llm_mechanisms:
                # Ensure X_original is set for acceptance set
                if is_deepchem and kwargs.get("X_original", None) is None and acceptance_set_X_original is not None:
                    kwargs["X_original"] = acceptance_set_X_original
                
                # Extract k_shot from kwargs if present
                k_shot = kwargs.get("k_shot", getattr(maicl, "k_shot", 0))
                # Use LLM-only evaluation for acceptance decisions
                # Pass all kwargs except those that evaluate_llm_only doesn't accept
                # Also exclude k_shot since we'll pass it explicitly
                llm_only_kwargs = {k: v for k, v in kwargs.items() if k not in ["relax_routing", "attention_temp", "min_ml_weight", "max_ml_weight", "hard_ml_gate_threshold", "preserve_mechanism_performance", "preserved_few_shot_examples", "k_shot"]}
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"[Experiment-specific] Using LLM-only evaluation for acceptance set ({getattr(maicl, '_acceptance_set_name', 'unknown')})")
                return _orig_evaluate_llm_only(X, y_true, X_pool, y_pool, k_shot=k_shot, **llm_only_kwargs)
            else:
                # No LLM mechanisms yet (e.g., initial checkpoint) - use regular evaluation
                return _orig_evaluate(X, y_true, X_pool, y_pool, *args, **kwargs)
        else:
            # Use regular evaluation for other cases (e.g., initial checkpoint, final evaluation)
            return _orig_evaluate(X, y_true, X_pool, y_pool, *args, **kwargs)

    maicl.evaluate = _evaluate_with_smiles_autofill_and_llm_only  # type: ignore[assignment]

    # Add ESOL-specific sign validation for mechanism updates
    # This prevents accepting updates with wrong signs (which cause negative R²)
    if str(args.dataset).lower() in ("esol", "delaney"):
        _orig_textgrad_optimize = None
        if hasattr(maicl, 'textgrad') and hasattr(maicl.textgrad, 'optimize'):
            _orig_textgrad_optimize = maicl.textgrad.optimize
            
            def _textgrad_optimize_with_sign_validation(*args, **kwargs):
                """Wrapper that validates signs for ESOL before optimization."""
                result = _orig_textgrad_optimize(*args, **kwargs)
                
                # Validate signs in the optimized mechanism
                if result and isinstance(result, (list, tuple)) and len(result) > 0:
                    for mech_text in result:
                        if isinstance(mech_text, str):
                            formula = extract_formula_from_llm_mechanism(mech_text)
                            if formula:
                                formula = _sanitize_formula_single_line(formula)
                                is_valid, error_msg = _validate_esol_signs(formula, args.dataset)
                                if not is_valid:
                                    import logging
                                    logger = logging.getLogger(__name__)
                                    logger.warning(f"  ⚠️  Sign validation failed: {error_msg}")
                                    logger.warning(f"  ⚠️  This mechanism update will likely be rejected")
                
                return result
            
            if _orig_textgrad_optimize:
                maicl.textgrad.optimize = _textgrad_optimize_with_sign_validation
    ...
    # Generate dataset-specific initial LLM mechanism before training to establish a better baseline
    # This avoids the poor performance of the placeholder "known" mechanism which has wrong signs
    initial_llm_mechanisms_count = len([t for t in maicl.mechanism_types if t == "llm"])
    if initial_llm_mechanisms_count == 0:
        import logging
        logger = logging.getLogger(__name__)
        dataset_key_lower = str(args.dataset).lower()
        logger.info(f"[Experiment-specific] Creating dataset-specific initial LLM mechanism for {args.dataset} (replacing placeholder known mechanism)...")
        
        # Create a dataset-specific initial mechanism with correct signs
        if dataset_key_lower in ("esol", "delaney"):
            # ESOL/Solubility: hydroxyl/oxygen/nitrogen positive, halogens/rings/mw negative
            # IMPROVED: Better balanced coefficients for standalone formula performance
            # Increased positive coefficients and adjusted intercept for better R²
            initial_mechanism = f"""[KNOWN] [KNOWN] DATASET CONTEXT:
Regression dataset: ESOL (Aqueous Solubility). This dataset predicts continuous numeric values from input features.

TASK: Regression task using all available features.

 Input: SMILES strings (molecular structures). The ML model uses ECFP fingerprints (1024 bits internally), but YOU should work with SMILES strings and molecular properties, NOT ECFP bit features.

[LLM] MECHANISM DESCRIPTION:
This mechanism predicts aqueous solubility based on molecular properties. Hydroxyl groups, oxygen, and nitrogen atoms increase solubility through hydrogen bonding with water. Halogens, rings, and molecular weight decrease solubility by increasing hydrophobicity.

STEP 1 - FEATURE TRANSFORMATIONS:
- normalized_molecular_weight = molecular_weight(SMILES) / 300
- saturated_hydroxyl = num_hydroxyl_groups(SMILES) / (1 + 0.15 * num_hydroxyl_groups(SMILES))

STEP 2 - INTERMEDIATE CONCEPTS:
- PolarityScore: Represents hydrophilic contributions from oxygen, nitrogen, and hydroxyl groups.
  Formula: PolarityScore = 0.4 * saturated_hydroxyl + 0.12 * num_oxygen(SMILES) + 0.12 * num_nitrogen(SMILES) + 0.08 * num_hbd(SMILES)
- HydrophobicityPenalty: Represents hydrophobic contributions from rings, halogens, and molecular weight.
  Formula: HydrophobicityPenalty = 0.15 * num_rings(SMILES) + 0.15 * is_aromatic(SMILES) + 0.12 * num_halogen(SMILES) + 0.004 * normalized_molecular_weight

STEP 3 - COMBINING INTERMEDIATES:
combined_signal = 0.55 + PolarityScore - HydrophobicityPenalty

STEP 4 - FINAL PREDICTION:
Formula: ŷ = clip(0.55 + 0.4 * (num_hydroxyl_groups(SMILES) / (1 + 0.15 * num_hydroxyl_groups(SMILES))) + 0.12 * num_oxygen(SMILES) + 0.12 * num_nitrogen(SMILES) + 0.08 * num_hbd(SMILES) - 0.15 * num_rings(SMILES) - 0.15 * is_aromatic(SMILES) - 0.12 * num_halogen(SMILES) - 0.004 * molecular_weight(SMILES) / 300, 0.0, 1.0)

ASSUMPTIONS / INTUITION:
- Hydroxyl groups, oxygen, and nitrogen increase solubility (positive coefficients).
- Halogens, rings, and molecular weight decrease solubility (negative coefficients).
- The intercept (0.55) provides a baseline near the middle of the [0,1] scaled range.
- Saturation function for hydroxyl groups prevents extreme predictions.
- Increased coefficients for better standalone formula performance.

RDKitValidation:
{{"expected_effects":{{"num_rings":"negative","is_aromatic":"negative","num_halogen":"negative","molecular_weight":"negative","num_atoms":"negative","num_oxygen":"positive","num_nitrogen":"positive","num_hydroxyl_groups":"positive","num_hbd":"positive"}}}}
"""
        elif dataset_key_lower in ("lipo", "lipophilicity"):
            # LogP/Lipophilicity: halogens/rings/mw positive, hydroxyl/oxygen/nitrogen negative
            initial_mechanism = f"""[KNOWN] [KNOWN] DATASET CONTEXT:
Regression dataset: LogP (Lipophilicity). This dataset predicts continuous numeric values from input features.
...
[LLM] MECHANISM DESCRIPTION:
This mechanism predicts lipophilicity (LogP) based on molecular properties. Halogens, rings, and molecular weight increase lipophilicity. Hydroxyl groups, oxygen, and nitrogen decrease lipophilicity by increasing hydrophilicity.

STEP 1 - FEATURE TRANSFORMATIONS:
- normalized_molecular_weight = molecular_weight(SMILES) / 300
- saturated_hydroxyl = num_hydroxyl_groups(SMILES) / (1 + 0.2 * num_hydroxyl_groups(SMILES))

STEP 2 - INTERMEDIATE CONCEPTS:
- HydrophobicityScore: Represents lipophilic contributions from rings, halogens, and molecular weight.
  Formula: HydrophobicityScore = 0.2 * num_rings(SMILES) + 0.15 * num_halogen(SMILES) + 0.002 * normalized_molecular_weight
- PolarityPenalty: Represents hydrophilic contributions from oxygen, nitrogen, and hydroxyl groups.
  Formula: PolarityPenalty = 0.3 * saturated_hydroxyl + 0.1 * num_oxygen(SMILES) + 0.1 * num_nitrogen(SMILES)

STEP 3 - COMBINING INTERMEDIATES:
combined_signal = 0.45 + HydrophobicityScore - PolarityPenalty

STEP 4 - FINAL PREDICTION:
Formula: ŷ = clip(0.45 + 0.2 * num_rings(SMILES) + 0.15 * num_halogen(SMILES) + 0.002 * molecular_weight(SMILES) / 300 - 0.3 * (num_hydroxyl_groups(SMILES) / (1 + 0.2 * num_hydroxyl_groups(SMILES))) - 0.1 * num_oxygen(SMILES) - 0.1 * num_nitrogen(SMILES), 0.0, 1.0)

ASSUMPTIONS / INTUITION:
- Halogens, rings, and molecular weight increase lipophilicity (positive coefficients).
- Hydroxyl groups, oxygen, and nitrogen decrease lipophilicity (negative coefficients).

RDKitValidation:
{{"expected_effects":{{"num_rings":"positive","is_aromatic":"positive","num_halogen":"positive","molecular_weight":"positive","num_atoms":"positive","num_oxygen":"negative","num_nitrogen":"negative","num_hydroxyl_groups":"negative","num_hbd":"negative"}}}}
"""
        else:
            # Fallback: generate via LLM with improved prompts
            logger.info(f"  Generating initial mechanism via LLM for dataset {args.dataset}...")
            try:
                initial_mechanisms = maicl.mech_generator.generate_unknown_mechanisms(
                    X_topk, y_topk, prediction_errors, ml_residuals=None
                )
                if initial_mechanisms and len(initial_mechanisms) > 0:
                    initial_mechanism = initial_mechanisms[0]
                else:
                    raise ValueError("LLM generation returned no mechanisms")
            except Exception as e:
                logger.warning(f"  ⚠️  Failed to generate initial mechanism via LLM: {e}, using placeholder")
                initial_mechanism = None
        
        if initial_mechanism:
            try:
                # Replace the known mechanism with our dataset-specific initial mechanism
                maicl.mech_generator.unknown_mechanisms = [initial_mechanism]
                maicl.mechanisms = maicl.mech_generator.get_all_mechanisms()
                maicl.mechanism_types = maicl.mech_generator.get_mechanism_types()
                logger.info(f"  ✓ Created dataset-specific initial LLM mechanism for {args.dataset} with correct signs")
            except Exception as e:
                import traceback
                logger.warning(f"  ⚠️  Failed to set initial mechanism: {e}")
    ...
    if not mechs:
        ...
        if best_iter == 0:
            logger.warning(f"  ⚠️  mechanisms_iter_0.txt not found, trying mechanisms_iter_1.txt as fallback")
            mechanisms_file = os.path.join(out_dir, f"mechanisms_iter_1.txt")
            mechs = parse_mechanisms_file(mechanisms_file)
            if mechs:
                best_iter = 1
                logger.info(f"  ✓ Using mechanisms from iteration 1 instead")
        if not mechs:
    ...
    for mech_type, mech_text in mechs:
        ...
        if str(args.dataset).lower() in ("esol", "delaney"):
            ...
            if not is_valid_signs:
                import logging
                logger = logging.getLogger(__name__)
                logger.debug(f"  Skipping mechanism with sign violations: {sign_error}")
                continue
    ...
    # CRITICAL: Evaluate using MA-ICL's full system (same as training) for fair comparison
    # This uses routing, ensemble, etc., not just the raw formula
    import logging
    logger = logging.getLogger(__name__)
    logger.info("\n" + "=" * 80)
    logger.info("FINAL MA-ICL SYSTEM EVALUATION (using full system, not just raw formula)")
    logger.info("=" * 80)
    
    # Evaluate on validation set using MA-ICL's full system
    maicl_val_metrics = maicl.evaluate(
        X_val, y_val_s, X_topk, y_topk,
        return_details=True,
        relax_routing=False,  # Use final routing weights from best snapshot
        preserve_mechanism_performance=True,  # Use preserved performance from best iteration
        X_original=X_original_val,
        X_pool_original=X_original_topk,
    )
    
    # Evaluate on test set using MA-ICL's full system
    maicl_test_metrics = maicl.evaluate(
        X_test, y_test_s, X_topk, y_topk,
        return_details=True,
        relax_routing=False,  # Use final routing weights from best snapshot
        preserve_mechanism_performance=True,  # Use preserved performance from best iteration
    ...
    logger.info(f"MA-ICL System (validation): R2={maicl_val_metrics.get('r2', 0):.4f}, MAE={maicl_val_metrics.get('mae', 0):.4f}")
    logger.info(f"MA-ICL System (test): R2={maicl_test_metrics.get('r2', 0):.4f}, MAE={maicl_test_metrics.get('mae', 0):.4f}")
    logger.info("=" * 80)
    logger.info("NOTE: The formula-only metrics below are for RDKit validation purposes.")
    logger.info("      The MA-ICL system metrics above show the actual system performance.")
    logger.info("=" * 80 + "\n")
    
    # Evaluate chosen mechanism on test split:
    # - Always compute RDKitValidation sign-match report (if present)
    # - Compute formula metrics if formula exists (for RDKit validation, not system performance)
    ...
    metrics_dict = {
        ...
        "formula": best_mech["formula"],
        # MA-ICL system metrics (full system with routing, ensemble, etc.)
        "maicl_system_val_metrics": {
            "r2": maicl_val_metrics.get("r2"),
            "mae": maicl_val_metrics.get("mae"),
            "mse": maicl_val_metrics.get("mse"),
            "rmse": maicl_val_metrics.get("rmse"),
        } if maicl_val_metrics else None,
        "maicl_system_test_metrics": {
            "r2": maicl_test_metrics.get("r2"),
            "mae": maicl_test_metrics.get("mae"),
            "mse": maicl_test_metrics.get("mse"),
            "rmse": maicl_test_metrics.get("rmse"),
        } if maicl_test_metrics else None,
    ...
    # Print MA-ICL system metrics (actual system performance)
    print("\n--- MA-ICL System Performance (full system with routing, ensemble, etc.) ---")
    if maicl_val_metrics:
        print("Validation set (MA-ICL system):")
        for k in ["r2", "mae", "mse", "rmse"]:
            if k in maicl_val_metrics:
                print(f"  {k}: {maicl_val_metrics[k]:.6f}")
    if maicl_test_metrics:
        print("Test set (MA-ICL system):")
        for k in ["r2", "mae", "mse", "rmse"]:
            if k in maicl_test_metrics:
                print(f"  {k}: {maicl_test_metrics[k]:.6f}")
    
    # Print formula-only metrics (for RDKit validation)
    print("\n--- Formula-Only Performance (raw formula, for RDKit validation) ---")
    print("NOTE: These metrics evaluate only the extracted formula, not the full MA-ICL system.")
    print("      The MA-ICL system metrics above show the actual system performance.")
